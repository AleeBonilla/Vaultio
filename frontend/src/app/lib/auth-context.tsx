...
interface AuthContextValue {
  ...
  updateProfile: (input: Partial<Pick<VaultioUser, "firstName" | "lastName" | "bio">> & { careerIds?: number[] }) => Promise<VaultioUser>;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isFirebaseConfigured) {
      setConfigError(
        "Firebase no está configurado. Copia frontend/.env.example a frontend/.env.local y completá las variables VITE_FIREBASE_*.",
      );
      setLoading(false);
      return;
    }
  ...
  const updateProfile = useCallback(
    async (input: Partial<Pick<VaultioUser, "firstName" | "lastName" | "bio">> & { careerIds?: number[] }) => {
