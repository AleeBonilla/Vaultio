...
const TYPE_ICONS: Array<{ match: string[]; icon: LucideIcon; classes: string }> = [
  ...
  {
    match: ["ejercicio", "practica", "práctica"],
    icon: FileQuestion,
  ...
  {
    match: ["presentacion", "presentación", "diapositiva"],
    icon: Presentation,
...
export function resourceTypeIcon(type: string) {
  ...
  return (
    TYPE_ICONS.find((item) => item.match.some((word) => normalized.includes(word))) || {
      icon: FileText,
...
const FILE_ICON_BY_EXTENSION: Record<string, string> = {
  ...
  gif: "/archivo-imagen.png",
  html: "/archivo-codigo.png",
  jpeg: "/archivo-imagen.png",
  jpg: "/archivo-imagen.png",
  java: "/archivo-codigo.png",
  js: "/archivo-codigo.png",
  json: "/archivo-codigo.png",
  png: "/archivo-imagen.png",
  svg: "/archivo-imagen.png",
  webp: "/archivo-imagen.png",
  link: "/link.png",
  md: "/archivo-txt.png",
  pdf: "/archivo-pdf.png",
  ppt: "/archivo-ppt.png",
...
export function ResourceTypeIcon({
  ...
  if (fileIcon) {
    return (
      <span
        className={`inline-flex items-center justify-center rounded-xl bg-white p-1.5 shadow-sm ring-1 ring-blue-100 ${className}`}
      >
